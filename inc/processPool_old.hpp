...
inline bool ProcessPool::Create(int procCount, int maxConcurrentProcs/*=0*/)
{
    ...
    int totalChildren = procCount;
    int maxConcurrentChildren = (maxConcurrentProcs > 0 ? maxConcurrentProcs : procCount);
...
//   <process id> and "crashed status" of the completed child
//   0  if all children completed
inline pid_t ProcessPool::WaitForChild(bool* isCrashed)
{
...
//    return true;
    ...
    // kill -0 checks if the process exists and we have permission
    if(kill(pid, 0) == 0)
        return true;

    // If kill fails with ESRCH, the process is definitely gone
    return (errno != ESRCH);
}

//
// TaskPool: A specialized ProcessPool for functional task execution.
// This class extends the base ProcessPool to allow passing a callable 
...
public:
    ...
    // a boolean (e.g., bool task(int index)). 
    ...
    bool Create(Func task, int procCount, int maxConcurrentProcs = 0)
    {
        ...
        if(IsChild())
        {
            try
            {
                bool res = task(GetChildIndex()); // Run the user-provided function
                Exit(res);      // Exit the child process using the result as the status
            }
            catch(...)
            {
                Exit(false);    // If the user's task throws, exit child with failure status
            }
        }
